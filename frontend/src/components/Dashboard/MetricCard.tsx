...
const changeClasses: Record<string, string> = {
 ...
 neutral: 'text-gray-500 dark:text-themeWhite/60 font-semibold',
};
...
export default function MetricCard({
 ...
 <p className="text-sm font-bold tracking-widest text-gray-600 dark:text-themeWhite/70 mb-2 truncate">
 {title}
 </p>
 <p className="text-4xl font-bold text-themeBlack dark:text-themeWhite truncate tracking-tighter">{value}</p>
 {change ? (
 <p className={`text-sm mt-3 ${changeClasses[changeType]} truncate tracking-normal`}>{change}</p>
 ) : null}
 </div>
