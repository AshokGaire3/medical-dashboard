...
export function VitalsTab({ patientId }: { patientId: number }) {
 ...
 <p className="text-xs font-medium text-gray-500 dark:text-themeWhite/60 mb-2">
 Last 30 days
 </p>
 ...
 className="p-1.5 rounded-md text-gray-600 dark:text-themeWhite/70 hover:bg-gray-100 dark:hover:bg-gray-800"
 >
...
function Th({ children, className = '' }: { children: React.ReactNode; className?: string }) {
 return (
 <th
 className={`px-4 py-2 text-left text-[11px] font-medium text-gray-500 dark:text-themeWhite/60 tracking-normal ${className}`}
 >
 {children}
 </th>
 );
...
function LiveIndicator({ state }: { state: HubConnectionState }) {
 ...
 className={`inline-flex items-center gap-1 text-[10px] font-medium tracking-wide px-1.5 py-0.5 rounded-full ${
 connected
