...
export function HealthScoreCard({ patientId }: { patientId: number }) {
 ...
 <p className="text-sm text-gray-500 dark:text-themeWhite/60">Computing health score…</p>
 </div>
 );
 }
 ...
 <p className="text-sm text-gray-500 dark:text-themeWhite/60">{message}</p>
 </div>
 );
 }
 ...
 <span className={`text-[10px] tracking-wide ${styles.text} opacity-80`}>
 {score.bandLabel}
 </span>
 </div>
 </div>
 ...
 >
 <p className={`text-[10px] tracking-wide ${styles.text} opacity-70`}>
