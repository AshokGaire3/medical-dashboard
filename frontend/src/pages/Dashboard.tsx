...
export default function Dashboard() {
 ...
 <h3 className="text-xl font-bold text-themeBlack dark:text-themeWhite tracking-tight">
 Today's appointments
 </h3>
 <Link
 to="/appointments"
 className="inline-flex items-center gap-1 text-sm font-bold text-accentBlue hover:underline tracking-wide"
 >
 ...
 <p className="text-sm text-gray-500 dark:text-themeWhite/60">No upcoming appointments.</p>
 ) : (
 <ul className="divide-y divide-gray-100 dark:divide-gray-800">
 {(upcomingQ.data ?? []).slice(0, 6).map((a) => (
 <li key={a.id} className="py-4 flex items-center justify-between">
 <div>
 <p className="text-base font-bold text-themeBlack dark:text-themeWhite">
 {a.patientName ?? `Patient #${a.patientId}`}
 </p>
 <p className="text-sm font-semibold text-gray-500 dark:text-themeWhite/60 mt-1">{a.reason}</p>
 </div>
 <span className="text-sm font-bold text-themeBlack dark:text-themeWhite tracking-widest">
 ...
 <h3 className="text-xl font-bold text-themeBlack dark:text-themeWhite tracking-tight">Critical alerts</h3>
 </div>
 {alertsQ.isLoading ? (
 <Spinner size="sm" />
 ) : alerts.length === 0 ? (
 <p className="text-sm text-gray-500 dark:text-themeWhite/60">
 ...
 <span
 className={`text-xs px-3 py-1 font-bold tracking-normal border-2 border-current ${
 a.severity === 'Critical'
 ...
 <p className="text-sm font-semibold tracking-normal mt-2 opacity-80">
 {a.vital}: {a.value}
 </p>
 </div>
 </button>
